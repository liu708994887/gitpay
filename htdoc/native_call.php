...
*/
    ...
    include_once($base_path . "class/WxPayPubHelper/WxPayPubHelper.php");
    ...
    $ret = file_put_contents('test.log', $get, FILE_APPEND);
	...
	if($nativeCall->checkSign() == FALSE){
		...
		switch ($product_id) 
		{
			case WxPayConf_pub::APPID."static"://与native_call_qrcode.php中的静态链接二维码对应
				...
				//sign已填,商户无需重复填写
                file_put_contents('test.log', '为什么呢？');
			...
			default:
                ...
                file_put_contents('test.log', date("Y-m-d H:i:s", time()));
				$nativeCall->setReturnParameter("return_code","OK");//返回状态码
				//$nativeCall->setReturnParameter("result_code","SUCCESS");//业务结果
